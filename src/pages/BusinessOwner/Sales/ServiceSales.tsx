...
const ServiceSales: FC<GraphsProps> = ({ stores, isLoading, isError }) => {
      ...
      const result = stores.reduce(
        (accum: any, store: any) => {
          ...
          store.invoices.forEach((invoice: any) => {
            // Check if there is a service sale in the invoice
            if (invoice.serviceSaleId !== 'no-sale' && invoice.serviceSale) {
              ...
              if (differenceInDays <= numberOfDays) {
                // Add each service as a separate object, combined with invoice data
                invoice.serviceSale.services.forEach((service: any) => {
                  accum.services.push({
                    ...service,
